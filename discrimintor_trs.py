...
def pixel_upsample(x, H, W):
    ...
    x = x.view(-1, C, H * W)
    x = x.permute(0, 2, 1)
    self.reduction = nn.Linear(scale * scale * dim, 0.5 * scale * scale * dim, bias=False)
    return x, H, W
...
class Upsample_layer(nn.Module):
    ...
    def __init__(self, input_resolution, dim, output_dim, norm_layer=nn.LayerNorm):
        super().__init__()
        self.input_resolution = input_resolution
        self.dim = dim
        # print(dim, dim/4)
...
class PatchMerging_scale(nn.Module):
    ...
    def __init__(self, input_resolution, dim, norm_layer=nn.LayerNorm, scale=2):
        super().__init__()
        self.input_resolution = input_resolution
        self.dim = dim
        self.reduction = nn.Linear(scale * scale * dim, 0.5 * scale * scale * dim, bias=False)
    ...
    def forward(self, x):
        ...
        B, L, C = x.shape
        assert L == H * W, "input feature has wrong size"
        assert H % self.scale == 0 and W % self.scale == 0, f"x size ({H}*{W}) are not even."
...
class Generator_transformer_pathch2(nn.Module):
    ...
    def __init__(self, img_size=256, patch_size=2, in_chans=3,
                 num_classes=1000,
                 embed_dim=96, depths=[4,6], num_heads=[3,6],
...
class Generator_transformer_pathch4_8_3_48_3(nn.Module):
    ...
    def __init__(self, img_size=256, patch_size=4, in_chans=3,
                 num_classes=1000,
                 embed_dim=96, depths=[8,4,4], num_heads=[6,6,6],
...
class Generator_transformer_pathch4_6_3_48_3(nn.Module):
    ...
    def __init__(self, img_size=256, patch_size=4, in_chans=3,
                 num_classes=1000,
                 embed_dim=96, depths=[6,6,6], num_heads=[8,8,8],
...
class Generator_transformer_pathch4_844_48_3_nouplayer_server5_no_normalizaiton(nn.Module):
    r""" Swin Transformer
    Args:
        img_size (int | tuple(int)): Input image size. Default 224
        patch_size (int | tuple(int)): Patch size. Default: 4
    ...
    def __init__(self, img_size=256, patch_size=4, in_chans=3,
        ...
        self.final_upsample = nn.Sequential(
            ...
            Upsample_promotion(input_resolution=(patches_resolution[0], patches_resolution[1]),
                               dim=embed_dim * 2, norm_layer=norm_layer,norm_flag=0),
            Upsample_promotion(input_resolution=(patches_resolution[0] * 2, patches_resolution[1] * 2),
                               dim=embed_dim, norm_layer=norm_layer,norm_flag=0),
            #
            # Upsample_layer(input_resolution=(patches_resolution[0], patches_resolution[1]),
            #                    dim=embed_dim*2, output_dim =embed_dim, norm_layer=norm_layer),
            # Upsample_layer(input_resolution=(patches_resolution[0] * 2, patches_resolution[1] * 2),
...
class my_cnn(nn.Module):
    def __init__(self, in_size, out_size, normalize=True, dropout=0.0,downsample = None):
        ...
        if downsample != None:
            self.downsample = downsample(dim=in_size, norm_layer=nn.BatchNorm2d)
        else:
            self.downsample = None
...
class Generator_cnn_pathch4_844_48_3_nouplayer_server5(nn.Module):
    ...
    def __init__(self, img_size=256, patch_size=4, in_chans=3,
        ...
        self.layers = nn.ModuleList()
        for i_layer in range(self.num_layers):
            layer = my_cnn(in_size=int(embed_dim * 2 ** i_layer),
                           out_size=int(embed_dim * 2 ** i_layer),
                           dropout=0.0,
                           downsample=PatchMerging_cnn if (i_layer < self.num_layers - 1) else None,
                           )
            self.layers.append(layer)
        ...
        for i_layer in range(self.num_layers - 1):
            if i_layer == 0:
                layer = my_cnn(in_size=int(embed_dim * 2 ** (self.num_layers - 1 - i_layer)),
                           out_size=int(embed_dim * 2 ** (self.num_layers - 1 - i_layer)),
                           dropout=0.0,
                           downsample=Upsample_promotion_cnn,
                           )
            else:
                layer = my_cnn(in_size=int(embed_dim * 2 ** (self.num_layers - 1 - i_layer)*2),
                           out_size=int(embed_dim * 2 ** (self.num_layers - 1 - i_layer) * 2),
                           dropout=0.0,
...
class Generator_transformer_pathch4_8421_48_3_nouplayer_server5(nn.Module):
    ...
    def __init__(self, img_size=256, patch_size=4, in_chans=3,
                 num_classes=1000,
                 embed_dim=96, depths=[8, 4, 2,1], num_heads=[8, 8, 8,8],
                 window_size=8, mlp_ratio=4., qkv_bias=True, qk_scale=None,
                 drop_rate=0., attn_drop_rate=0., drop_path_rate=0,
                 norm_layer=nn.LayerNorm, ape=True, patch_norm=True,
                 use_checkpoint=False, **kwargs):
...
class Discriminator_transformer(nn.Module):
    ...
    def __init__(self, img_size=256, patch_size=4, in_chans=6,
                 num_classes=1,
